...
def large_dollar_volume_stocks(df, price_column, volume_column, top_percent):
    ...
    return (
        dollar_traded.sort_values()
        .tail(int(len(dollar_traded) * top_percent))
        .index.values.tolist()
    )
...
def plot_weights(weights, title):
    ...
    trace = _generate_heatmap_trace(
        weights.sort_index(axis=1, ascending=False),
        "Date",
        "Ticker",
        "Weight",
        0.0,
    ...
    layout = go.Layout(
        title=title, xaxis={"title": "Dates"}, yaxis={"title": "Tickers"}
    )
    ...
    display(
        HTML(
            'The graph for {} is too large. You can view it <a href="{}" target="_blank">here</a>.'.format(
                title, graph_path
            )
        )
    )
...
def plot_returns(returns, title):
    ...
    graph_path = "graphs/{}.html".format(_sanatize_string(title))
    trace = _generate_heatmap_trace(
        returns.sort_index(axis=1, ascending=False),
        "Date",
        "Ticker",
        "Weight",
        -0.3,
...
def plot_covariance_returns_correlation(correlation, title):
    ...
    xaxis2_layout = {"showgrid": False, "zeroline": False, "showticklabels": False}
...
def plot_xty(xty, title):
    ...
    layout = go.Layout(
        title=title, xaxis={"title": "Tickers"}, yaxis={"title": "Covariance"}
    )
