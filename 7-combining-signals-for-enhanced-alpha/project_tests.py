...
def test_calculate_oob_score(fn):
    ...
    x = np.arange(n_estimators * n_samples * n_features).reshape(
        [n_estimators, n_samples, n_features]
    )
    y = np.sum(x, axis=-1) + noise
    estimators = [
        RandomForestRegressor(300, oob_score=True, n_jobs=-1, random_state=101).fit(
            x[estimator_i], y[estimator_i]
        )
        for estimator_i in range(n_estimators)
    ]

    fn_inputs = {"classifiers": estimators}
...
def test_non_overlapping_estimators(fn):
    ...
    with patch.object(RandomForestRegressor, "fit", autospec=True) as mock_fit:
        ...
        assert_structure(
            fn_return_value, [RandomForestRegressor for _ in range(n_estimators)], "PCA"
        )
        ...
        for classifier in fn_return_value:
            try:
                ...
                raise Exception(
                    "Test Failure: RandomForestRegressor.fit not called on all classifiers"
                )
