...
def _generate_second_tetration_stock(stock_symbol, dates):
    ...
    sector_stock["base_line"] = (
        sector_stock["base_line"] + all_noise[0] * sector_stock["base_line"]
    )
    sector_stock["adj_open"] = (
        sector_stock["base_line"] + all_noise[1] * sector_stock["base_line"]
    )
    sector_stock["adj_close"] = (
        sector_stock["base_line"] + all_noise[2] * sector_stock["base_line"]
    )
    sector_stock["adj_high"] = (
        sector_stock["base_line"] + all_noise[3] * sector_stock["base_line"]
    )
    sector_stock["adj_low"] = (
        sector_stock["base_line"] + all_noise[4] * sector_stock["base_line"]
    )

    sector_stock["adj_high"] = sector_stock[["adj_high", "adj_open", "adj_close"]].max(
        axis=1
    )
    sector_stock["adj_low"] = sector_stock[["adj_low", "adj_open", "adj_close"]].min(
        axis=1
    )
...
def plot_signal(price, signal, title):
    ...
    layout = go.Layout(title=title, annotations=buy_annotations + sell_annotations)
...
def plot_signal_to_normal_histograms(signal_list, title, subplot_titles):
    ...
    for series_i, signal_series in enumerate(signal_series_list, 1):
        ...
        normal_trace = go.Histogram(
            x=np.random.normal(
                np.mean(filtered_series), np.std(filtered_series), len(filtered_series)
            ),
