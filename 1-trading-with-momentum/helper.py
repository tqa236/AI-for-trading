...
def download_quandl_dataset(
    quandl_api_key, database, dataset, save_path, columns, tickers, start_date, end_date
):
    """
    ...
    """
    scrape_url = "https://www.quandl.com/api/v3/datatables/{}/{}?qopts.export=true&api_key={}".format(
        database, dataset, quandl_api_key
    )
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        bulk_download_request = requests.get(
            bulk_download_url, stream=True, cookies=scrape_request.cookies
        )
        ...
        with tqdm(total=5, desc="Transforming Data", unit="Action") as pbar:
            ...
            assert (
                len(csv_files) == 1
            ), "Bulk download of Quandl Wiki data failed. Wrong number of csv files found. Found {} file(s).".format(
                len(csv_files)
            )
            ...
            tmp_df = tmp_df[
                tmp_df["date"].isin(pd.date_range(start_date, end_date))
            ]  # Filter unused dates
            tmp_df = tmp_df[tmp_df["ticker"].isin(tickers)]  # Filter unused tickers
            pbar.update(1)
            tmp_df.to_csv(
                save_path, columns=columns, index=False
            )  # Filter unused columns and save
            pbar.update(1)
