...
def _generate_traces(name_df_color_data):
    ...
    for name, df, color in name_df_color_data:
        traces.append(
            go.Scatter(name=name, x=df.index, y=df, mode="lines", line={"color": color})
        )
...
def plot_returns(returns, title):
    ...
    traces = _generate_traces([("Returns", returns, helper.color_scheme["major_line"])])
...
def print_top(df, name, top_n=10):
    ...
    print(
        ", ".join(df.sum().sort_values(ascending=False).index[:top_n].values.tolist())
    )
