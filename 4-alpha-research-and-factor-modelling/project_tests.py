...
def test_fit_pca(fn):
    ...
    with patch.object(PCA, "fit", autospec=True) as mock_fit:
        ...
        try:
            fn_return_value.fit.assert_called_with(
                self=fn_return_value, X=fn_inputs["returns"]
            )
        except Exception:
        ...
        if not does_data_match(
            fn_return_value.components_, fn_correct_values["PCA.components_"]
        ):
            raise Exception(
                "Test Failure: PCA not fitted correctly\n\n"
                "PCA.components_:\n"
                "{}\n\n"
    ...
    fn_correct_outputs = OrderedDict(
        [
            (
                ...
                pd.DataFrame(np.full([3, 3], 0.0), assets, assets),
            )
        ]
    )
...
def test_predict_portfolio_risk(fn):
    ...
    fn_inputs = {
        ...
        "idiosyncratic_var_matrix": pd.DataFrame(
            np.diag([0.02272535, 0.05190083, 0.03040361]), assets, assets
        ),
        "weights": pd.DataFrame([0.0, 0.0, 0.25], assets),
    }
    fn_correct_outputs = OrderedDict([("portfolio_risk_prediction", 0.0550369570517)])
...
def test_mean_reversion_5day_sector_neutral(fn):
    ...
    universe = AverageDollarVolume(window_length=universe_window_length).top(
    ...
    print(
        "> universe = AverageDollarVolume(window_length={}).top({})".format(
...
def test_sharpe_ratio(fn):
    ...
    fn_correct_outputs = OrderedDict(
        [
            (
                "sharpe_ratio",
                pd.Series([3.21339895, 12.59157330, 6.54485802], factor_names),
            )
        ]
    )
...
def test_optimal_holdings_get_obj(cl):
    ...
    fn_correct_outputs = OrderedDict(
        [
            (
                "solution",
                np.array(
                    [-3.33960455e-10, -2.75871416e-11, -5.00000000e-01, 5.00000000e-01]
                ),
            )
        ]
    )
    ...
    assert_output(
        solve_problem, fn_inputs, fn_correct_outputs, check_parameter_changes=False
    )
...
def test_optimal_holdings_get_constraints(cl):
    ...
    print(
        "> factor_betas = np.arange({} * {}).reshape([{}, {}])".format(
    ...
    fn_correct_outputs = OrderedDict(
        [
            (
                "solution",
                np.array(
                    [-2.80288449e-10, -4.73562710e-12, -5.12563104e-10, 7.97632862e-10]
                ),
            )
        ]
    )
