...
def _generate_output_error_msg(fn_name, fn_inputs, fn_outputs, fn_expected_outputs):
    ...
    for input_name, input_value in fn_inputs.items():
        formatted_outputs.append("INPUT {}:\n{}\n".format(input_name, str(input_value)))
...
def generate_random_tickers(n_tickers=None):
    ...
    ticker_symbol_random = np.random.randint(
        ord("A"), ord("Z") + 1, (n_tickers, max_ticker_len)
    )
    ticker_symbol_lengths = np.random.randint(min_ticker_len, max_ticker_len, n_tickers)
    for ticker_symbol_rand, ticker_symbol_length in zip(
        ticker_symbol_random, ticker_symbol_lengths
    ):
        ticker_symbol = "".join(
...
def assert_structure(received_obj, expected_obj, obj_name):
    ...
    ), "Wrong type for output {}. Got {}, expected {}".format(
        obj_name, type(received_obj), type(expected_obj)
    )
    ...
    if hasattr(expected_obj, "shape"):
        assert (
            ...
            obj_name, received_obj.shape, expected_obj.shape
        )
    elif hasattr(expected_obj, "__len__"):
        assert len(received_obj) == len(
            expected_obj
        ), "Wrong len for output {}. Got {}, expected {}".format(
            obj_name, len(received_obj), len(expected_obj)
        )
    ...
    if type(expected_obj) == pd.DataFrame:
        assert set(received_obj.columns) == set(expected_obj.columns), (
            ...
            "EXPECTED COLUMNS: {}".format(
                obj_name, sorted(received_obj.columns), sorted(expected_obj.columns)
            )
        )

        # This is to catch a case where __equal__ says it's equal between different types
        assert set([type(i) for i in received_obj.columns]) == set(
        ...
        for column in expected_obj.columns:
            assert received_obj[column].dtype == expected_obj[column].dtype, (
                ...
                "EXPECTED Type: {}".format(
                    obj_name,
                    column,
                    received_obj[column].dtype,
                    expected_obj[column].dtype,
                )
            )
    ...
    if type(expected_obj) in {pd.DataFrame, pd.Series}:
        assert set(received_obj.index) == set(expected_obj.index), (
            ...
            "INDICES:          {}\n"
            "EXPECTED INDICES: {}".format(
                obj_name, sorted(received_obj.index), sorted(expected_obj.index)
            )
        )

        # This is to catch a case where __equal__ says it's equal between different types
        assert set([type(i) for i in received_obj.index]) == set(
...
def assert_output(fn, fn_inputs, fn_expected_outputs, check_parameter_changes=True):
    ...
    if check_parameter_changes:
        for input_name, input_value in fn_inputs.items():
            passed_in_unchanged = _is_equal(
                input_value, fn_inputs_passed_in[input_name]
            )

            assert passed_in_unchanged, (
                'Input parameter "{}" has been modified inside the function. '
    ...
    err_message = _generate_output_error_msg(
        fn.__name__, fn_inputs, fn_outputs, fn_expected_outputs
    )

    for fn_out, (out_name, expected_out) in zip(
        fn_outputs.values(), fn_expected_outputs.items()
    ):
